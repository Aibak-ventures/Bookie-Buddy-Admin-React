...
import {
  addMainService,
  fetchMainServices,
  toggleMainServiceStatus,
  updateMainService,
  deleteMainService,
} from "../../api/AdminApis";
import AddMainServiceModal from "../Modals/AddMainServiceModal";
...
const MainServices = () => {
  ...
  const loadServices = async () => {
    ...
    setError(null);
    try {
      const data = await fetchMainServices(currentUrl);
  ...
  const handleUpdateService = async (serviceData) => {
    try {
      await updateMainService(serviceData.id, serviceData);
  ...
  const handleToggleStatus = async (id, isActive) => {
    try {
      await toggleMainServiceStatus(id, !isActive);
      setServices((prev) =>
        prev.map((service) =>
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-8">
      <div className="flex items-center justify-between mb-8">
        <h1 className="text-3xl font-bold text-gray-800">Main Services</h1>
        <button
